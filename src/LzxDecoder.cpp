...
LzxDecoder::LzxDecoder(const uint_fast16_t window_bits)
{
	...
	this->state_window_size = (uint32_t)(1 << window_bits);
	...
	{
		LzxDecoder::extra_bits[i] = LzxDecoder::extra_bits[i + 1] = (uint8_t)j;
		if((i != 0) && (j < 17))
		{
			++j;
		}
	...
	{
		LzxDecoder::position_base[i] = (uint32_t)j;
	...
	this->state_main_elements = (uint16_t)(NUM_CHARS + (posn_slots << 3));
...
void LzxDecoder::Decompress(const uint8_t* inBuf, uint_fast32_t inLen, uint8_t* outBuf, uint_fast32_t outLen)
{
	if(outLen < 1)
	{
		std::cerr << "LzxDecoder: outLen is < 1 (" << outLen << ")\n";
		throw -1;
	}
	...
	uint_fast32_t togo = outLen, this_run, main_element, match_length, match_offset, length_footer, extra, verbatim_bits;
	...
	while(togo > 0)
	{
		...
		if(this->state_block_remaining == 0)
		{
			...
			j = bitbuf->ReadBits(8);
			this->state_block_remaining = this->state_block_length = (uint32_t)((i << 8) | j);
			...
			switch(this->state_block_type)
			{
				case ALIGNED:
				{
					...
					{
						j = bitbuf->ReadBits(3);
						this->state_ALIGNED_len[i] = (uint8_t)j;
					}
				...
				default:
				{
					throw std::string("Invalid state block type");
				}
			}
		}
		...
		while((this_run = this->state_block_remaining) > 0 && togo > 0)
		{
			...
			switch(this->state_block_type)
			{
				...
				case UNCOMPRESSED:
				{
					...
					LzxDecoder::ArrayCopy(inBuf, bitbuf->inpos, this->state_window, window_posn, this_run);
	...
	LzxDecoder::ArrayCopy(this->state_window, start_window_pos, outBuf, 0, outLen);
...
uint32_t LzxDecoder::ReadHuffSym(uint16_t* table, uint8_t* lengths, uint32_t nsyms, uint32_t nbits, BitBuffer* bitbuf)
{
	uint32_t i, j;
	bitbuf->EnsureBits(16);
	if((i = table[bitbuf->PeekBits((uint8_t)nbits)]) >= nsyms)
	{
		j = (uint32_t)(1 << (int32_t)((sizeof(uint32_t) * 8) - nbits));
		do
		{
			j >>= 1; i <<= 1; i |= (bitbuf->buffer & j) != 0 ? (uint32_t)1 : 0;
			if(j == 0)
			{
				std::cerr << "LzxDecoder: j == 0 in ReadHuffSym\n";
...
void LzxDecoder::ArrayCopy(const uint8_t* arrayIn, uint_fast32_t inStart, uint8_t* arrayOut, uint_fast32_t outStart, uint_fast32_t length)
{
